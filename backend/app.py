...
def process_page(page):
    ...
    if table_objects:
        for table in table_objects:
            ...
            all_table_words = table_page.extract_words(use_text_flow=True)
            table_words = [word for word in all_table_words if word.get('upright', True)]
    ...
    all_page_words_unfiltered = page.extract_words()
    all_page_words = [word for word in all_page_words_unfiltered if word.get('upright', True)]
    
    non_table_words = [word for word in all_page_words if not any(char in table_chars for char in page.crop((word['x0'], word['top'], word['x1'], word['bottom'])).chars)]
    ...
    for block_words in final_blocks:
        ...
        if block_text.strip():
            ...
            x1 = max(w['x1'] for w in block_words)
            bottom = max(w['bottom'] for w in block_words)
            page_elements.append({"type": "text", "text": block_text, "bbox": [x0, top, x1, bottom]})

    page_elements.sort(key=lambda item: item['bbox'][1])
    ...
    try:
        ...
        with pdfplumber.open(temp_pdf_path) as pdf:
            for i, page in enumerate(pdf.pages):
                page_number = i + 1
                all_pages_data[page_number] = process_page(page)
